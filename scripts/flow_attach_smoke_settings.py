...
FLOW_UPLOAD_SETTLE_SECONDS = int(os.getenv("FLOW_UPLOAD_SETTLE_SECONDS", "12"))
...
def click_text_if_visible(page, text, label, wait_ms=1000):
    selectors = [
        f"text={text}",
    ...
    return click_first(page, selectors, label, wait_ms=wait_ms)


def print_visible_controls(page, limit=40):
    print("\nVisible controls:")
    controls = page.locator("button, [role='button'], [role='option'], [role='menuitem']")
    for i in range(min(controls.count(), limit)):
        try:
            c = controls.nth(i)
            if not c.is_visible():
                continue
            text = (c.inner_text(timeout=500) or "").strip()
...
def select_model(page):
    ...
    # If the model is already visible, try clicking it or assume selected.
    if click_text_if_visible(page, FLOW_IMAGE_MODEL, f"Model {FLOW_IMAGE_MODEL}", wait_ms=1000):
        print(f"Model selected/visible: {FLOW_IMAGE_MODEL}")
        return True

    # Try model dropdown controls.
    clicked_dropdown = click_first(
        page,
        [
            "button:has-text('Model')",
            "[role='button']:has-text('Model')",
            "[aria-label*='Model']",
    ...
    if clicked_dropdown and click_text_if_visible(page, FLOW_IMAGE_MODEL, f"Model option {FLOW_IMAGE_MODEL}", wait_ms=1500):
        print(f"Model selected: {FLOW_IMAGE_MODEL}")
        return True

    print(f"Model selection not confirmed: {FLOW_IMAGE_MODEL}")
    return False
...
def select_aspect_ratio(page):
    ...
    # Prefer exact ratio text.
    if click_text_if_visible(page, FLOW_ASPECT_RATIO, f"Aspect ratio {FLOW_ASPECT_RATIO}", wait_ms=1200):
        print(f"Aspect ratio selected/visible: {FLOW_ASPECT_RATIO}")
        return True

    # Try opening aspect dropdown.
    clicked_aspect = click_first(
        page,
        [
            "button:has-text('Aspect')",
            "[role='button']:has-text('Aspect')",
            "[aria-label*='Aspect']",
            "button:has-text('Ratio')",
            "[role='button']:has-text('Ratio')",
            "[aria-label*='Ratio']",
        ],
    ...
    if clicked_aspect and click_text_if_visible(page, FLOW_ASPECT_RATIO, f"Aspect option {FLOW_ASPECT_RATIO}", wait_ms=1200):
        print(f"Aspect ratio selected: {FLOW_ASPECT_RATIO}")
        return True

    print(f"Aspect ratio selection not confirmed: {FLOW_ASPECT_RATIO}")
    return False


def set_flow_generation_settings(page):
    print("Setting Flow generation settings...")
    ...
    print(f"Settings result: model_ok={model_ok}, aspect_ok={aspect_ok}")
...
def wait_for_upload_settle(page):
    print(f"Waiting {FLOW_UPLOAD_SETTLE_SECONDS}s for upload to complete in gallery...")
    page.wait_for_timeout(FLOW_UPLOAD_SETTLE_SECONDS * 1000)

    # Extra stabilization pass: avoid selecting older gallery images while upload is still rendering.
    last_count = -1
    stable_hits = 0

    for _ in range(10):
        try:
            count = page.locator("[role='button']:has(img), button:has(img), main img").count()
        except Exception:
            count = -1
        ...
        if count == last_count and count > 0:
            stable_hits += 1
            if stable_hits >= 2:
                print(f"Gallery/media count stabilized: {count}")
                return
        else:
            stable_hits = 0
            last_count = count
    ...
    print("Upload settle wait completed.")
...
def open_uploaded_media(page):
    ...
    # Give Flow time to render latest uploaded cards.
...
with sync_playwright() as p:
    ...
    set_flow_generation_settings(page)
    upload_images(page)
